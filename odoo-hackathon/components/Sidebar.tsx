...
export default function Sidebar({ isCollapsed, setIsCollapsed }: { isCollapsed: boolean, setIsCollapsed: (val: boolean) => void }) {
  ...
  return (
    <div className={`${isCollapsed ? 'w-[80px]' : 'w-[260px]'} bg-sidebar-bg h-screen flex flex-col fixed left-0 top-0 z-20 transition-all duration-300 border-r border-border/50`}>
      ...
      {/* Navigation */}
      <div className="px-3 overflow-y-auto flex-1 space-y-6 scrollbar-hide">
        <div>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 py-2.5 rounded-full text-sm font-medium transition-all duration-200 ${
                    isActive 
      ...
      {/* Footer Area */}
      <div className="px-3 pb-4 pt-2 mt-auto flex flex-col gap-3 items-center">
        {/* Quick Action Icons */}
        <div className={`flex items-center ${isCollapsed ? 'flex-col gap-3' : 'gap-1.5 px-2 w-full'}`}>
          {!isCollapsed && (
             <>
              <button className="p-2 text-muted hover:bg-surface-hover hover:text-foreground rounded-full transition-colors border border-transparent hover:border-border shrink-0" title="Settings">
